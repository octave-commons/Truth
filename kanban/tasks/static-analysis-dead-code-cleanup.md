...
write-id: "1784985273887-0.8lrsj9yzrztty5f1jfg"
...
Triage 2026-07-24 — COUNTS AND PARTITION BOTH STALE. Actual is **353**, not the 157 in §1 or the 312 above. More importantly the partition misses the dominant category: **199 of 353 are facade re-export aliases** (`infra.render` 28, `domain.planet-formation` 18, `domain.ecology` 18, `law.field` 16, `law.mass-transfer` 15, `domain.stellar.structure` 12, `infra.menu` 8, `infra.inspect` 8, `law.stellar` 27, rest) — surface generated by the *completed* split cards, fitting none of the four children. Two further corrections: (a) **zero vars qualify for `^:private`** — clojure-lsp only fires when there are no *cross-namespace* refs, so a var used inside its own ns is still flagged; all six non-facade candidates were hand-checked and are false matches (docstring prose, keyword literals, a `defprotocol` method). (b) **there is no `.lsp/config.edn` at all**, so 48 outright false positives (33 `defcomponent`/`defevent`-generated, 14 bench `ns-resolve` entry points, 1 protocol method) are unsuppressed; config-first collapses 261 of 353 without per-var review. Superseded by `static-analysis-lsp-config-dead-vars.md`, `static-analysis-facade-prune.md` and `static-analysis-unused-pending-convention.md` under `static-analysis-regression-2026-07-24.md`.
---

---
## Superseded, and its counts were stale (2026-07-25)

Superseded by `kanban/tasks/static-analysis-lsp-config-dead-vars.md` +
`static-analysis-facade-prune.md`, which drove unused-public-vars to **0**.

Two corrections, so the numbers here are not trusted later:

- **§1's "157 unused public vars across 30 namespaces" was stale.** The real figure
  when this work started was **353**. It regressed while this card sat `ready`.
- **The `^:private` bucket has ZERO members.** clojure-lsp's `unused-public-var`
  fires only when there are no *cross-namespace* references, so same-file usage is
  the discriminator; all candidates were hand-checked and are false matches
  (docstring prose, keyword literals, a protocol method). The four-bucket triage in
  §1 should be read as three buckets: delete, `^:export`, or `UNUSED-PENDING`.

See `kanban/tasks/static-analysis-regression-2026-07-24.md` §Wave 0.
---