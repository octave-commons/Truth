...
(defn can-afford-resonance? "True if the observer has at least `cost` resonance." [observer cost] (>= (double (or (:resonance observer) 0.0)) (double cost)))

(defn spend-resonance "Deduct `cost` resonance (clamped at zero). Caller should `can-afford-resonance?` first." [observer cost] (update observer :resonance (fn [r] (max 0.0 (- (double (or r 0.0)) (double cost))))))

(defn apply-coherence "Pure update of an observer's coherence. Drain and regen are per-frame\n   (not sim-time dependent), so the bar moves at a consistent wall-clock rate\n   regardless of simulation speed. Focus-intensity is the lever: high focus\n   drains fast, low focus lets coherence recover. Witnessing events gives bursts." [observer _dt _environmental-complexity witnessed-events] (let [fi (double (:focus-intensity observer 0.5)) drain (coherence-drain-from-focus fi) regen (coherence-regen-rate fi) gains (reduce + 0.0 (map (fn* [p1__244#] (coherence-gain-from-event p1__244# (:coherence observer))) witnessed-events)) coherence' (-> (:coherence observer) (- drain) (+ regen) (+ gains) (max 0.0) (min (:max-coherence observer)))] (-> observer (assoc :coherence coherence') (update :resonance-events (fn* [p1__245#] (vec (take 100 (concat witnessed-events p1__245#))))))))
