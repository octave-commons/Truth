...
(defn- traverse-stack
  "Explicit-stack Barnes–Hut traversal with primitive scalar accumulators.
   ...
   local state is mutated. Pairs closer than the cutoff radius contribute zero
   acceleration."
  [G soft2 cutoff2 theta2 px py pz self-id tree]
  (if (nil? tree)
    [0.0 0.0 0.0]
    (let [G       (double G)
          soft2   (double soft2)
...
(defn acceleration-for-soa
  ...
  ([G theta softening soa _self-id]
   (acceleration-for-soa G theta softening 0.0 soa _self-id))
  ([G theta softening cutoff soa _self-id]
...
(defn acceleration
  ...
  ([G tree body]
   (acceleration G default-theta default-softening 0.0 tree body))
  ([G theta tree body]
   (acceleration G theta default-softening 0.0 tree body))
  ([G theta softening tree body]
   (acceleration G theta softening 0.0 tree body))
  ([G theta softening cutoff tree body]
   (let [pos   (:position body)
         [px py pz] pos
