#!/usr/bin/env bash
set -euo pipefail
# Polls inbox for truth-research-coordinator and dispatches via eta-mu for each new message.
# Run in tmux/screen/systemd for a long-lived actor.
ACTOR_ID="truth-research-coordinator"
...
while true; do
  for msg in "$INBOX"/*.md; do
    ...
    "/home/err/.agents/skills/eta-mu-actor-agent/scripts/dispatch-actor-eta-mu.sh" "$ACTOR_ID" "$body"
    mv "$msg" "$msg.done"
