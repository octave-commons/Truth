...
(defn- spark-observer
  "A minimal observer map for spring-step tests: stationary at `pos`, zero
   spark-velocity."
  [pos]
  {:position pos :spark-velocity (sp/vec3 0.0 0.0 0.0)})

(defn- run-steps
  "Run `spark-binding-step` `n` times at fixed `target` / `strength` / `dt`,
   returning the seq of resulting observers (not including the initial one)."
  [observer target strength dt n]
  (rest (take (inc n) (iterate #(narrowing/spark-binding-step % target strength dt) observer))))

(deftest spring-pulls-stationary-spark-toward-fixed-target
  (testing "a stationary spark under a bound tether approaches a fixed target
            monotonically, never overshooting past it"
    (let [target (sp/vec3 1.0e11 0.0 0.0)
          start  (spark-observer (sp/vec3 0.0 0.0 0.0))
          dists  (map #(sp/dist (:position %) target)
                      (run-steps start target 1.0 0.05 40))]
      (is (apply >= dists) "distance to target is non-increasing every step")
      (is (< (last dists) (first dists)) "the spark actually gets closer")
      (is (every? #(<= % (sp/dist (:position start) target)) dists)
          "no step ever diverges past the starting distance"))))

(deftest stronger-binding-approaches-faster
  (testing "a higher tether-strength closes more distance in the same number
            of steps than a lower one"
    (let [target (sp/vec3 1.0e11 0.0 0.0)
          start  (spark-observer (sp/vec3 0.0 0.0 0.0))
          weak   (last (run-steps start target 0.2 0.05 20))
          strong (last (run-steps start target 1.0 0.05 20))]
      (is (< (sp/dist (:position strong) target)
             (sp/dist (:position weak) target))
          "stronger binding closes more of the gap in the same time"))))

(deftest zero-binding-spark-is-free
  (testing "a stationary spark at zero strength is not pulled toward a distant
            target at all — it stays put"
    (let [target (sp/vec3 1.0e11 0.0 0.0)
          start  (spark-observer (sp/vec3 0.0 0.0 0.0))
          after  (last (run-steps start target 0.0 0.05 20))]
      (is (= (:position start) (:position after))
          "no spring force, no initial velocity => the free spark does not move"))))

(deftest player-input-active-frame-spring-does-not-override
  (testing "observer-motion-step returns the observer unchanged when the
            player is actively flying this frame, even under full binding"
    (let [target (sp/vec3 1.0e11 0.0 0.0)
          start  (spark-observer (sp/vec3 0.0 0.0 0.0))
          after  (narrowing/observer-motion-step
                  start {:target-pos target :strength 1.0 :dt 0.05
                         :input-active? true})]
